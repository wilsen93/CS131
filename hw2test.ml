...
let test_1 = ((parse_prefix new_grammar accept_all ["+"]) = Some ([ (A, [N B]) ; (B, [T "+"]) ], []))

type new_nonterminals2 = 
| E | F | G | H
...
let test_2 = ((parse_prefix new_grammar2 accept_all ["+";"1"; "-"]) = Some ([ (E, [N F; T "1"]) ; (F, [T "+"]) ], ["-"]))